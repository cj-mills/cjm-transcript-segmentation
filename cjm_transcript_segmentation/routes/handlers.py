...
# %% auto #0
__all__ = ['DEBUG_SEG_HANDLERS', 'build_mutation_response', 'SegInitResult', 'SegMutationResult', 'init_workflow_router']
...
class SegMutationResult(NamedTuple):
    ...
    """
    segment_dicts: List[Dict[str, Any]]  # Segments after mutation
    focused_index: int  # Focused segment index after mutation
    visible_count: int  # Visible card count
    history_depth: int  # Undo history depth after mutation
    is_auto_mode: bool  # Whether card count is in auto-adjust mode
    extra_oob: tuple = ()  # Additional OOB elements (e.g., merge rejection flash)
...
async def _handle_seg_split_result(
    ...
    # Validate index — return unchanged segments on invalid
    if segment_index < 0 or segment_index >= len(ctx.segment_dicts):
        return SegMutationResult(
            ctx.segment_dicts, ctx.focused_index, ctx.visible_count,
            len(ctx.history), ctx.is_auto_mode,
        )
    ...
    # Can't split at beginning or end — return unchanged
    if char_position <= 0 or char_position >= len(segment.text):
        return SegMutationResult(
            ctx.segment_dicts, segment_index, ctx.visible_count,
            history_depth, ctx.is_auto_mode,
        )
    ...
    return SegMutationResult(
        new_segment_dicts, new_focused_index, ctx.visible_count,
        history_depth, ctx.is_auto_mode,
    )
...
async def _handle_seg_split(
    ...
    result = await _handle_seg_split_result(
        state_store, workflow_id, request, sess, segment_index, urls, max_history_depth,
    )
...
def _handle_seg_merge(
    ...
    result = _handle_seg_merge_result(
        state_store, workflow_id, request, sess, segment_index, urls, max_history_depth,
    )
    return (*build_mutation_response(
        result.segment_dicts, result.focused_index, result.visible_count,
...
def _handle_seg_undo_result(
    ...
    return SegMutationResult(
        previous_segments, new_focused_index, ctx.visible_count,
        len(remaining_history), ctx.is_auto_mode,
    )
...
def _handle_seg_reset_result(
    ...
    return SegMutationResult(
        initial_segments, 0, ctx.visible_count,
        history_depth, ctx.is_auto_mode,
    )
...
def _handle_seg_reset(
    ...
    result = _handle_seg_reset_result(
        state_store, workflow_id, request, sess, urls, max_history_depth,
    )
    return (*build_mutation_response(
        result.segment_dicts, result.focused_index, result.visible_count,
        result.history_depth, urls, is_auto_mode=result.is_auto_mode,
...
async def _handle_seg_ai_split_result(
    ...
    if not ctx.segment_dicts:
        return SegMutationResult(
            [], 0, ctx.visible_count, len(ctx.history), ctx.is_auto_mode,
        )
    ...
    return SegMutationResult(
        new_segment_dicts, 0, ctx.visible_count,
        history_depth, ctx.is_auto_mode,
    )
...
async def _handle_seg_ai_split(
    ...
    result = await _handle_seg_ai_split_result(
        state_store, workflow_id, segmentation_service, request, sess, urls, max_history_depth,
    )
    return (*build_mutation_response(
        result.segment_dicts, result.focused_index, result.visible_count,
        result.history_depth, urls, is_auto_mode=result.is_auto_mode,
